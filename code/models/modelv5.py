...
class cnnModel(nn.Module):
    def __init__(self,color_channels=1,units=2,dim=64,frames=8):
        ...
        self.invNet1 = nn.Sequential(
           nn.Conv2d(in_channels=3, out_channels=frames,kernel_size=5, stride=1, padding=(5-1)//2),
           nn.LeakyReLU(inplace=True),
           nn.Conv2d(in_channels=frames, out_channels=frames,kernel_size=3, stride=1, padding=(3-1)//2),
           nn.LeakyReLU(inplace=True)
        )

        self.invNet2 = nn.Sequential(
           nn.Conv2d(in_channels=3, out_channels=frames,kernel_size=5, stride=1, padding=(5-1)//2),
           nn.LeakyReLU(inplace=True),
           nn.Conv2d(in_channels=frames, out_channels=frames,kernel_size=3, stride=1, padding=(3-1)//2),
           nn.LeakyReLU(inplace=True)
